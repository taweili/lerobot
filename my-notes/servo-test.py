...
def test_servo_communication(port='/dev/ttyACM0', motor_id=1, torque=None):
    ...
    try:
        ...
        # Handle torque setting
        if torque is not None:
            torque_value = 1 if torque == 'on' else 0
            motor_bus.write('Torque_Enable', torque_value, motor_name)
            print(f"Torque {'enabled' if torque_value else 'disabled'}")
        else:
            # Default behavior: enable torque for testing
        ...
        # Disable torque when done if not explicitly set by user
        if torque is None:
            motor_bus.write('Torque_Enable', 0, motor_name)
            print(f"Disable torque")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--torque', choices=['on', 'off'],
                       help='Enable or disable servo torque (default: enabled for testing, disabled after)')
    args = parser.parse_args()

    if args.id:
        # Test single specified servo
        test_servo_communication(port=args.port, motor_id=args.id, torque=args.torque)
    else:
        # Default behavior: test all servos 1-7
        for motor_id in range(1, 8):
            try:
                test_servo_communication(port=args.port, motor_id=motor_id, torque=args.torque)
            except Exception:
